...
export async function gradeAndStoreExamSubmission({
  ...
  const deleteAnswersQuery = db.delete(examAnswers).where(eq(examAnswers.sessionId, sessionId));
  const updateSessionQuery = db
    ...
    .where(and(eq(studentSessions.id, sessionId), isNull(studentSessions.completedAt)));

  if (recordsToInsert.length > 0) {
    const insertAnswersQuery = db.insert(examAnswers).values(recordsToInsert);
    await db.batch([deleteAnswersQuery, insertAnswersQuery, updateSessionQuery]);
  } else {
    await db.batch([deleteAnswersQuery, updateSessionQuery]);
  }
